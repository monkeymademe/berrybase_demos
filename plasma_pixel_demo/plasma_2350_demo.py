...
def set_pixel(index, color):
    """Set a pixel color using Plasma library's set_rgb method."""
    led_strip.set_rgb(index, color[0], color[1], color[2])
...
def animation_rainbow(duration):
    ...
    while time() - start_time < duration:
        for i in range(num_pixels):
            ...
            set_pixel(i, apply_brightness(color, brightness))
        offset = (offset + 1) % 256
        sleep(0.01)
    ...
    while time() - start_time < duration:
        for pos in range(num_pixels):
            ...
            for trail in range(1, trail_length + 1):
                ...
                set_pixel(trail_pos, trail_color)
...
def animation_sparkle(duration):
    ...
    while time() - start_time < duration:
        ...
        for pos in sparkle_positions:
            set_pixel(pos, sparkle_color)
        
        sleep(0.2)  # Brief flash
        
        # Turn off the sparkles
        for pos in sparkle_positions:
            set_pixel(pos, off_color)
...
def animation_larson_scanner(duration):
    ...
    while time() - start_time < duration:
        ...
        for i in range(scanner_width):
            ...
            set_pixel(led_pos, scanner_color)
